...
if ~iscolumn(galLabels)
    galLabels = galLabels'; % column
end

if ~isrow(probLabels)
    probLabels = probLabels'; % row
end
...
[~, sortedIndex] = sort(score, 'descend'); % rank the score, along dim 1
score(binaryLabels == false) = -Inf; % set scores of non-matches to -Inf
clear binaryLabels
[~, maxIndex] = max(score); % get the location of the maximum genuine score 最大的真实分数
